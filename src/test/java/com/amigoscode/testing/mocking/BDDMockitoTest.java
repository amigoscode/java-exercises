package com.amigoscode.testing.mocking;
...
class BDDMockitoTest {
    ...
    @Test
    @DisplayName("BDD given/willReturn and then/should")
    void testBDDGivenWillReturn() {
        // Given
        Order order = new Order("ORD-1", "CUST-1", 99.99);
        given(paymentService.charge("CUST-1", 99.99)).willReturn(true);
        // When
    ...
    @Test
    @DisplayName("BDD willThrow for exception stubbing")
    void testBDDWillThrow() {
        // Given
        Order order = new Order("ORD-2", "CUST-2", 50.0);
        given(paymentService.charge(anyString(), anyDouble()))
            .willThrow(new RuntimeException("Payment gateway down"));
    ...
    @Test
    @DisplayName("Complete BDD-style test")
    void testCompleteBDD() {
        // Given
        Order order = new Order("ORD-3", "CUST-3", 75.0);
        given(paymentService.charge("CUST-3", 75.0)).willReturn(true);
        // When
    ...
    @Test
    @DisplayName("BDD with any() matchers")
    void testBDDWithAnyMatchers() {
        given(paymentService.charge(anyString(), anyDouble())).willReturn(true);
        orderService.placeOrder(new Order("ORD-1", "CUST-1", 50.0));
        orderService.placeOrder(new Order("ORD-2", "CUST-2", 75.0));
    ...
    @Test
    @DisplayName("Traditional Mockito style")
    void testTraditionalStyle() {
        Order order = new Order("ORD-4", "CUST-4", 100.0);
        willReturn(true).given(paymentService).charge("CUST-4", 100.0);
        Order result = orderService.placeOrder(order);
        assertEquals("COMPLETED", result.getStatus());
        then(paymentService).should().charge("CUST-4", 100.0);
        then(orderRepository).should().save(order);
    }

    @Test
    @DisplayName("BDD Mockito style")
    void testBDDStyle() {
        // Given
        Order order = new Order("ORD-4", "CUST-4", 100.0);
        given(paymentService.charge("CUST-4", 100.0)).willReturn(true);
        // When
        Order result = orderService.placeOrder(order);
        // Then
        assertEquals("COMPLETED", result.getStatus());
